...
#define DEBUG_TYPE "tinygpu-lower"
...
TINYGPUTargetLowering::TINYGPUTargetLowering(const TargetMachine &TM,
  ...
  addRegisterClass(MVT::i8, &TINYGPU::GPRRegClass);

  // Compute derived properties from the register classes.
  computeRegisterProperties(STI.getRegisterInfo());

  // TODO: Set physical register or not?
  setStackPointerRegisterToSaveRestore(TINYGPU::R1);
...
SDValue TINYGPUTargetLowering::LowerFormalArguments(
  ...
  CCInfo.AnalyzeFormalArguments(Ins, CC_TINYGPU8); // FIXME
  ...
  for (auto &VA : ArgLocs) {
    ...
    const unsigned VReg =
      RegInfo.createVirtualRegister(&TINYGPU::GPRRegClass);
...
TINYGPUTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
  ...
  CCInfo.AnalyzeReturn(Outs, RetCC_TINYGPU8);
  ...
  return DAG.getNode(TINYGPUISD::END_PGM, DL, MVT::Other, RetOps);
}

const char *TINYGPUTargetLowering::getTargetNodeName(unsigned Opcode) const {
  switch ((TINYGPUISD::NodeType)Opcode) {
  case TINYGPUISD::FIRST_NUMBER:
    break;
  case TINYGPUISD::END_PGM:
    return "TINYGPUISD::END_PGM";
  }
  return nullptr;
}