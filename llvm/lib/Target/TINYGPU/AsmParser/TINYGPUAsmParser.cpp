...
struct TINYGPUOperand : public MCParsedAsmOperand {
...
public:
    TINYGPUOperand(const TINYGPUOperand &o) : MCParsedAsmOperand() {
        Kind = o.Kind;
        StartLoc = o.StartLoc;
        EndLoc = o.EndLoc;
        switch (Kind) {
    ...
    static std::unique_ptr<TINYGPUOperand> createToken(StringRef Str, SMLoc S) {
        auto Op = std::make_unique<TINYGPUOperand>(Token);
        Op->Tok = Str;
        Op->StartLoc = S;
        Op->EndLoc = S;
        return Op;
    ...
    static std::unique_ptr<TINYGPUOperand> createReg(unsigned RegNo, SMLoc S, SMLoc E) {
        auto Op = std::make_unique<TINYGPUOperand>(Register);
        Op->Reg.RegNum = RegNo;
        Op->StartLoc = S;
        Op->EndLoc = E;
        return Op;
    ...
    static std::unique_ptr<TINYGPUOperand> createImm(const MCExpr *Val, SMLoc S, SMLoc E) {
        auto Op = std::make_unique<TINYGPUOperand>(Immediate);
        Op->Imm.Val = Val;
        Op->StartLoc = S;
        Op->EndLoc = E;
        return Op;
...
bool TINYGPUAsmParser::MatchAndEmitInstruction(SMLoc IDLoc, unsigned &Opcode,
    ...
    switch (MatchInstructionImpl(Operands, Inst, ErrorInfo, MatchInlineAsm)) {
    default:
        break;
    case Match_Success:
        Inst.setLoc(IDLoc);
...
OperandMatchResultTy TINYGPUAsmParser::parseImmediate(OperandVector &Operands) {
  ...
  SMLoc E = SMLoc::getFromPointer(S.getPointer() - 1);
  Operands.push_back(TINYGPUOperand::createImm(IdVal, S, E));
  return MatchOperand_Success;
}
...
bool TINYGPUAsmParser::parseOperand(OperandVector &Operands) {
...
bool TINYGPUAsmParser::ParseDirective(AsmToken DirectiveID) { return true; }

extern "C" void LLVMInitializeTINYGPUAsmParser() {
  RegisterMCAsmParser<TINYGPUAsmParser> X(getTheTINYGPUTarget());
}